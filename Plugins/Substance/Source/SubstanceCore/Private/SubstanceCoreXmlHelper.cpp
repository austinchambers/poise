	...
	const graph_desc_t* graphDesc)
{
	...
	for (const XMLNode* child = RootNode->GetFirstChildNode(); child != NULL;child = child->GetNextNode())
	{
		...
		// Parse preset
		preset.mPackageUrl = 
			sbspresetnode->GetAttribute("pkgurl");
		preset.mPackageUrl = graphDesc!=NULL ?
			graphDesc->PackageUrl :
			preset.mPackageUrl;
		preset.mLabel = 
			sbspresetnode->GetAttribute("label");
		preset.mDescription = 
			sbspresetnode->GetAttribute("description");

		
		// Loop on preset inputs
		for (const XMLNode* presetinputnode = sbspresetnode->GetFirstChildNode(); presetinputnode != NULL;presetinputnode = presetinputnode->GetNextNode())
		{
